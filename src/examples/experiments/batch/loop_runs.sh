...
for exp_len_par in $experiment_length; do
    ...
    for comm_type in $comm_type_set; do
        ...
        for priority_k_par in $priority_k_list; do
            ...
            comm_root_dir=$priority_dir
            for comm_par in $rebroadcast_list; do
                comm_dir=$comm_root_dir/"Rebroadcast#"$comm_par
                if [[ ! -e $comm_dir ]]; then
                    mkdir $comm_dir
                fi
                if [[ $comm_type == "anon" ]]; then
                                                ...
                                                for voting_msgs_par in $voting_msgs; do
                                                    ...
                                                    for control_parameter_par in $control_parameter; do
                                                        ...
                                                        for i in $(seq 1 $RUNS); do
                                                            kilo_file="run#${i}.tsv"
                                                            config=`printf 'config_nrobots%d_comm%s_rebroad%d_adap%d_priorityk%d_MsgExpTime%d_MsgHop%d_Opt%d_Eta%s_InitDistr%s_Ctrl%s_Vote%d_CPar%s_run%d.argos' $agents_par $comm_type $comm_par $adaptive_par $priority_k_par $msgs_par $msgs_hop_par $options_par $eta_par $init_par $control_par $voting_msgs_par $control_parameter_par $i`
