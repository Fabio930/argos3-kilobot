                        ...
                        for k_par in $k_sampling; do
                            ...
                            if [[ -e $hops_dir ]]; then
                                ...
                                for i in $(seq 1 $RUNS); do
                                    kilo_file="run#${i}.tsv"
                                    config=`printf 'config_nrobots%s_rebroad%d_MsgExpTime%s_Thr%s_Gt%s_Ksamp%s_run%s.argos' $agents_par $comm_par $msgs_par $thr_par $dlt_par $k_par $i`
