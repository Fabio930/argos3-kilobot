...
for exp_len_par in $experiment_length; do
    ...
    for comm_type in $comm_type_set; do
        comm_type_dir=$exp_len_dir/"CommType#"$comm_type
        if [[ ! -e $comm_type_dir ]]; then
            mkdir $comm_type_dir
        fi
        if [[ $comm_type == "anon" ]]; then
            id_aware_val=0
            for priority_k_par in $priority_k_set; do
                priority_dir=$comm_type_dir/"PriorityK#"$priority_k_par
                if [[ ! -e $priority_dir ]]; then
                    mkdir $priority_dir
                fi
                for comm_par in 0; do
                    comm_dir=$priority_dir/"Rebroadcast#"$comm_par
                    if [[ ! -e $comm_dir ]]; then
                        mkdir $comm_dir
                    fi
                    msgs_n_hops="0"
                    for adaptive_par in $adaptive_set; do
                        if [[ $adaptive_par != "0" ]]; then
                            continue
                        fi
                                                    ...
                                                    for voting_msgs_par in $voting_msgs; do
                                                        ...
                                                        for control_parameter_par in $control_parameter; do
                                                            ...
                                                            for i in $(seq 1 $RUNS); do
                                                                kilo_file="run#${i}.tsv"
                                                                config=`printf 'config_nrobots%d_comm%s_rebroad%d_adap%d_idaware%d_priorityk%d_MsgExpTime%d_MsgHop%d_Opt%d_Eta%s_InitDistr%s_Ctrl%s_Vote%d_CPar%s_run%d.argos' $agents_par $comm_type $comm_par $adaptive_par $id_aware_val $priority_k_par $msgs_par $msgs_hop_par $options_par $eta_par $init_par $control_par $voting_msgs_par $control_parameter_par $i`
            ...
            for comm_par in $rebroadcast; do
                ...
                for adaptive_par in $adaptive_set; do
                    if [[ $comm_par == "0" && $adaptive_par != "0" ]]; then
                        continue
                    fi
                    adaptive_dir=$comm_dir/"Adaptive#"$adaptive_par
                    if [[ ! -e $adaptive_dir ]]; then
