            ...
            if '.' not in folder and "images" not in folder:
                ...
                for minus in MINS:
                    for thr in self.thresholds:
                        results[(max_steps,commit,minus,thr)] = (self.compute_states(bigM_1,bigM_2,minus,thr),bigM_1,bigM_2)
                print("")
        print("DONE\n")
        return results,COMMIT,MAX_STEPS,MINS
    
##########################################################################################################
    def print_mean_quorum_value(self,data_in,BASE,COMMUNICATION,N_AGENTS,COMMIT,MAX_STEPS,MINS):
        COMMIT,MAX_STEPS, MINS = np.sort(COMMIT),np.sort(MAX_STEPS),np.sort(MINS)
        ...
        for m in range(len(MINS)):
            for t in range(len(self.thresholds)):
                for s in MAX_STEPS:
                    we_will_print=False
                    to_print = [[]]*len(data_in.get((s,COMMIT[0],MINS[m],self.thresholds[t])))
                    legend = [[]]*len(data_in.get((s,COMMIT[0],MINS[m],self.thresholds[t])))
                    for r in COMMIT:
                        for l in range(len(data_in.get((s,r,MINS[m],self.thresholds[t])))):
                            if (print_only_state or l==0) and (data_in.get((s,r,MINS[m],self.thresholds[t])))[l] is not None:
                                # print(s,r,"\n")
                                we_will_print=True
                                bigM = (data_in.get((s,r,MINS[m],self.thresholds[t])))[l]
                        ...
                        for l in range(len(to_print)):
                            if (print_only_state or l==0):
                                ...
                                if l==0:
                                    ...
                                    fig_path=BASE+"/Rebroadcast#"+str(COMMUNICATION)+"/Robots#"+str(N_AGENTS)+"/images/quorum/CONFIGs__COMM#"+str(COMMUNICATION)+"_ROB#"+str(N_AGENTS)+"_MINl#"+str(MINS[m])+"_THR#"+str(self.thresholds[t])+"_STEPS#"+str(s).replace(".","-")+".png"
                                    plt.yticks(np.arange(0,1.05,0.05))
                                elif l==1:
                                    plt.ylabel("average quorum length")
                                    fig_path=BASE+"/Rebroadcast#"+str(COMMUNICATION)+"/Robots#"+str(N_AGENTS)+"/images/quorum/CONFIGql__COMM#"+str(COMMUNICATION)+"_ROB#"+str(N_AGENTS)+"_STEPS#"+str(s).replace(".","-")+".png"
                                    plt.yticks(np.arange(0,N_AGENTS+1,1))
                                elif l==2:
                                    plt.ylabel("average quorum level")
                                    fig_path=BASE+"/Rebroadcast#"+str(COMMUNICATION)+"/Robots#"+str(N_AGENTS)+"/images/quorum/CONFIGqv__COMM#"+str(COMMUNICATION)+"_ROB#"+str(N_AGENTS)+"_STEPS#"+str(s).replace(".","-")+".png"
                                    plt.yticks(np.arange(0,N_AGENTS+1,1))
...
##########################################################################################################
    def print_single_run_quorum(self,data_in,BASE,COMMUNICATION,N_AGENTS,COMMIT,MAX_STEPS,MINS,position='first',taken="all"):
        ...
        for m in range(len(MINS)):
            for t in range(len(self.thresholds)):
                for s in MAX_STEPS:
                    we_will_print = False
                    to_print = [[]]*len(data_in.get((s,COMMIT[0],MINS[0],self.thresholds[0])))
                    legend = [[]]*len(data_in.get((s,COMMIT[0],MINS[0],self.thresholds[0])))
                    p,P = 0,0
                    for r in COMMIT:
                        if P==0 and position!='first' and taken=="all":
                            P = 1
                            if position=='rand': p = np.random.choice(np.arange(len(data_in.get((s,r,MINS[m],self.thresholds[t]))[0])))
                            elif position=='last': p = len(data_in.get((s,r,MINS[m],self.thresholds[t]))[0])-1
                        for l in range(len(data_in.get((s,r,MINS[m],self.thresholds[t])))):
                            if(print_only_state or l==0) and  data_in.get((s,r,MINS[m],self.thresholds[t]))[l] is not None:
                                we_will_print=True
                                run = data_in.get((s,r,MINS[m],self.thresholds[t]))[l][p]
                                mean = [-1]*len(run[0])
                                flag = [mean]*(len(run)+1)
                    ...
                    if we_will_print:
                        for l in range(len(to_print)):
                            if (print_only_state or l==0):
                                ...
                                if l==0:
                                    plt.ylabel("average swarm state")
                                    fig_path=BASE+"/Rebroadcast#"+str(COMMUNICATION)+"/Robots#"+str(N_AGENTS)+"/images/quorum/srCONFIGs__COMM#"+str(COMMUNICATION)+"_ROB#"+str(N_AGENTS)+"_MINl#"+str(MINS[m])+"_THR#"+str(self.thresholds[t])+"_STEPS#"+str(s)+"_Nrun#"+str(p)+".png"
                                    plt.yticks(np.arange(0,1.05,0.05))
                                elif l==1:
                                    plt.ylabel("average quorum length")
                                    fig_path=BASE+"/Rebroadcast#"+str(COMMUNICATION)+"/Robots#"+str(N_AGENTS)+"/images/quorum/srCONFIGql__COMM#"+str(COMMUNICATION)+"_ROB#"+str(N_AGENTS)+"_STEPS#"+str(s)+"_Nrun#"+str(p)+".png"
                                    plt.yticks(np.arange(0,N_AGENTS+1,1))
                                elif l==2:
                                    plt.ylabel("average quorum level")
                                    fig_path=BASE+"/Rebroadcast#"+str(COMMUNICATION)+"/Robots#"+str(N_AGENTS)+"/images/quorum/srCONFIGqv__COMM#"+str(COMMUNICATION)+"_ROB#"+str(N_AGENTS)+"_STEPS#"+str(s)+"_Nrun#"+str(p)+".png"
                                    plt.yticks(np.arange(0,N_AGENTS+1,1))
                                plt.legend(handles=handls.tolist(),loc='best')
                                plt.tight_layout()
                                plt.savefig(fig_path)
